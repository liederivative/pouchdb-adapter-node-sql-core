...
function WebSqlPouch(opts, callback) {
  ...
  function parseTypeArray(array) {
    ...
    // console.log([ ...numbers, strings ], '$IN ')
    return [].concat(numbers, strings);
  }
  function getSort(options) {
    let sort = options.sort;
    if(typeof sort === 'object'){
      if(sort[0]){
        return { sort: Object.keys(sort[0])[0], direction: Object.keys(sort[0])[1] }
      }
    }
  ...
  function parseOptsToQuery(logicalOperator, field, value){
    console.log(`${JSON.stringify(logicalOperator||'')}, ${JSON.stringify(field ||'')}, ${JSON.stringify(value ||'')}`, );
    if(typeof logicalOperator === 'undefined' || !logicalOperator) return;
    let jsonFunction = 'json_extract';
    let useLength = (field.split('.')[1] === 'length');
    if (useLength) field = field.split('.')[0];
    switch(logicalOperator){
      case '$gt':
      return `${useLength?'json_array_length(':''}json_extract(A.json,'$.${field}')${useLength?')':''} > ${parseType(nulltoEmpty(value.$gt) ) }`;
      case '$gte':
      return `${useLength?'json_array_length(':''}json_extract(A.json,'$.${field}')${useLength?')':''} >= ${parseType(nulltoEmpty(value.$gte ) )}`;
      case '$lt':
      return `${useLength?'json_array_length(':''}json_extract(A.json,'$.${field}')${useLength?')':''} < ${parseType(nulltoEmpty(value.$lt) ) }`;
      case '$lte':
      return `${useLength?'json_array_length(':''}json_extract(A.json,'$.${field}')${useLength?')':''} <= ${parseType(nulltoEmpty(value.$lte ) )}`;
      case '$in':
        // if(value.$in.length === 0) return;
        let inValues = parseTypeArray(value.$in);
        if(field === '_id'){
          return `A.doc_id IN (${inValues.join(',')})`;
        }else{
          return `json_extract(A.json,'$.${field}') IN (${inValues.join(',')})`;
        }
      case '$or':
        ...
        for (var i = value.length - 1; i >= 0; i--) {
          var opt = value[i];
          var field = Object.keys(opt)[0];
          var newValue = Object.values(opt)[0];
          var operator = Object.keys(newValue)[0];
          console.log(field, value, operator,newValue[operator], 'FIELD, VALUE, OPERATOR,value[opt]');
          var parseOpt = parseOptsToQuery(operator,field,newValue);
          new_values.push(parseOpt);
        }
        var parseNewValues = new_values.join(' OR ')
        return `(${parseNewValues})`;
        // let inValues = parseTypeArray(value.$in);
        // if(field === '_id'){
        //   return `A.doc_id IN (${inValues.join(',')})`;
        // }
      return `json_extract(A.json,'$.${field}') IN (${inValues.join(',')})`;
      case '$ne':
      return `json_extract(A.json,'$.${field}') <> ${parseType(value.$ne)}`;
      case '$regex':
        if(!value.$regex) return;
        if(field === '_id'){
          return `A.doc_id LIKE '%${value.$regex}%'`;
        }
      return `json_extract(A.json,'$.${field}') LIKE '%${nulltoEmpty(value.$regex)}%'`;
      case '$not_like':
      if(field === '_id'){
          return `A.doc_id NOT LIKE '%${value.$not_like}%'`;
        }
      return `json_extract(A.json,'$.${field}') NOT LIKE '%${value.$not_like}%'`;
      case '$exists':
      return (value.$exists)?
          `A.json LIKE '%"${field}"%'`
          :`A.json NOT LIKE '%"${field}"%'`;
      case '$not':
      return `A.json NOT LIKE '%"${field}":${value.$not}%'`
    }
  }
  // 0:id input, 2:callback error
  api._query = function(id, opts, callback){
    var tx = opts.ctx;
    if (!tx) {
      return db.readTransaction(function (txn) {
        api._query(id, pouchdbUtils.assign({ctx: txn}, opts), callback);
      });
    }
    var limit = opts.limit ? opts.limit : 1000;
    var orderby = opts.orderby?opts.orderby:'date';
    let { sort, direction } = getSort(opts);
    var startkey = opts.startkey ? opts.startkey : false;
    var startkey_docid = opts.startkey_docid ? opts.startkey_docid : false;
    var selector = opts.selector || {};
    if(!selector) {
      callback( pouchdbErrors.createError(pouchdbErrors.QUERY_PARSE_ERROR, 'selector is required') );
      return;
    }
    ...
    for(var i = 0; i < fields.length; i++){
      ...
      var operators = typeof value ==='object' ? Object.keys(value || {}): [];
      if(Array.isArray(value)) operators = [field];
      console.log(field,operators)
      if( operators.length ){
        for (var o = 0; o < operators.length; o++) {
          let parsedResult = parseOptsToQuery(operators[o], field, value);
          if(parsedResult) conditions.push(parsedResult);
        }
      }else{
        var jsonObj = {};
        if(field === '_id'){
          if(typeof value !== 'object'){
            conditions.push(`A.doc_id = '${value}'`)
          }
        }else{
          jsonObj[field] = value;
          var stringLike = JSON.stringify(jsonObj).replace('{','').replace('}','')
          conditions.push(`A.json LIKE '%${stringLike}%'`)
        }
    ...
    var sql = 
    `SELECT A.json,A.doc_id,A.rev FROM 'by-sequence' AS A 
    INNER JOIN "document-store" AS B ON B.winningseq = A.seq
    WHERE A.deleted = 0 AND A.json NOT LIKE '{"language":"query"%' `;
    
    var part2 = `AND ${conditions}
    ${startkey_docid?`AND seq >= (select seq from 'by-sequence' where doc_id = '${startkey_docid}')`:''}
    ORDER BY json_extract(A.json,'$.${(sort)?sort:orderby}') ${direction?direction:'DESC'}
    LIMIT ${limit};`
    if(conditions) sql = sql + ' '+ part2;
    console.log(`QUERY -: ${sql}`);
    tx.executeSql(sql, [], function (a, results) {
      // console.log(results)
      ...
      for(var i = 0; i < results.rows.length; i++){
        ...
        if( item.hasOwnProperty('json')){
          var result = pouchdbJson.safeJsonParse( item.json );
          result._id = item.doc_id;
          result._rev = item.rev;
          parseResult.push( result  );
        }
      }
      // console.log(parseResult);
      return callback('',{docs: parseResult});
