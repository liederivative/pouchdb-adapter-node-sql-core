...
function WebSqlPouch(opts, callback) {
  ...
  function parseOptsToQuery(logicalOperator, field, value){
    ...
    switch(logicalOperator){
      case '$gt':
      return `json_extract(A.json,'$.${field}') > '${nulltoEmpty(value.$gt)}'`;
      case '$gte':
      return `json_extract(A.json,'$.${field}') >= '${nulltoEmpty(value.$gte)}'`;
      case '$lt':
      return `json_extract(A.json,'$.${field}') < '${nulltoEmpty(value.$lt)}'`;
      case '$lte':
      return `json_extract(A.json,'$.${field}') <= '${nulltoEmpty(value.$lte)}'`;
      case '$in':
        if(value.$in.length === 0) return;
        let inValues = parseType(value.$in);
      return `json_extract(A.json,'$.${field}') IN (${inValues.join(',')})`;
      case '$ne':
      return `json_extract(A.json,'$.${field}') <> '${value.$ne}'`;
      case '$regex':
      ...
      case '$not_like':
      return `json_extract(A.json,'$.${field}') NOT LIKE '%${value.$regex}%'`;
      case '$exists':
      return (value.$exists)?
          `A.json LIKE '%"${field}"%'`
