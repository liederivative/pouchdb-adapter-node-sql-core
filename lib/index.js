...
function WebSqlPouch(opts, callback) {
  ...
  function parseOptsToQuery(logicalOperator, field, value){
    switch(logicalOperator){
      case '$gt':
      return `json_extract(A.json,'$.${field}') > '${value.$gt}'`;
      case '$gte':
      return `json_extract(A.json,'$.${field}') >= '${value.$gte}'`;
      case '$lt':
      return `json_extract(A.json,'$.${field}') < '${value.$lt}'`;
      case '$lte':
      return `json_extract(A.json,'$.${field}') <= '${value.$lte}'`;
      case '$in':
      return `json_extract(A.json,'$.${field}') IN (${value.$in.join(',')})`;
      case '$exists':
      return (value.$exists)?
          `A.json LIKE '%"${field}"%'`
  ...
  api._query  = function(id, opts, callback){
    ...
    for(var i = 0; i < fields.length; i++){
      ...
      if( operators.length ){
        var newConditions = operators.map( operator => 
          parseOptsToQuery(operator, field, value) )
    ...
    ORDER BY json_extract(A.json,'$.${orderby}') DESC
    LIMIT ${limit};`
    console.log(sql);
